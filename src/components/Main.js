...
class Main extends Component {
    ...
    render() {
        const {satInfo, isLoadingList, selected, satList, time, isLoadingPosition, positions} = this.state
        return (
            <Row className="main">
                <Col span={8} className="left-side">
                    <SatSetting onShow={this.showNearbySatellite}/>
                    <SatelliteList satInfo={satInfo} isLoad={isLoadingList} selected={selected}
                                   onShowMap={this.showMap} onUpdateSelectedList={this.updateSelectedList}/>
                </Col>
                <Col span={16} className="right-side">
                    <WorldMap satData={satList} time={time} isLoading={isLoadingPosition} positions={positions}/>
                </Col>
            </Row>
        )
    }
